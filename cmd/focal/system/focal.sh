...
DESC="Shell authenticated reverse proxy"
NAME=focal
DIRECTORY=/etc/focal/directory.yml
PORT=9666
DAEMON=/usr/local/bin/$NAME
DAEMON_ARGS="--directory ${DIRECTORY} --port ${PORT}"
...
do_start()
{
	...
	start-stop-daemon --start --quiet --pidfile $PIDFILE --exec $DAEMON --test > /dev/null \
		|| return 1
	start-stop-daemon --start -b --quiet --pidfile $PIDFILE --exec /usr/bin/env FOCAL_TOKEN="J6FHrHEFRokgh2"  $DAEMON -- \
		$DAEMON_ARGS \
		|| return 2
